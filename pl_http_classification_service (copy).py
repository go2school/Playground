		...
		if self.path.find('?') > -1:						
			...
			if bad_request == True:
				...
				if body == '':					
					...
					mec[var_title] = title
					mec[var_body] = body				
					mec.execute('from svm_server_util import from_title_body_to_svm_vector, multi_label_prediction_with_platts_calibration_feature_shrinkage')																			
