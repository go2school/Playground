...
def setup_tree_view(my_str_output, node, all_nodes, format_type, id2cat, is_expanded):
	my_str_output['str'] = '<a href=\\"javascript:ddtreemenu.flatten(\'treeresults\', \'expand\')\\">Expand All</a> | <a href=\\"javascript:ddtreemenu.flatten(\'treeresults\', \'contact\')\\">Contact All</a>'	
	print_HTML_tree_view(my_str_output, 0, node, all_nodes, format_type, id2cat)	
	my_str_output['str'] += '<script type=\\"text/javascript\\">'
	my_str_output['str'] += 'ddtreemenu.createTree(\\"treeresults\\", true);';
	if is_expanded == True:
		my_str_output['str'] += 'ddtreemenu.flatten(\\"treeresults\\", \\"expand\\");</script>';
	else:
		my_str_output['str'] += 'ddtreemenu.flatten(\\"treeresults\\", \\"contact\\");</script>';
	
def print_HTML_tree_view(my_str_output, depth, node, all_nodes, format_type, id2cat):
	...
	if m != 0:	
		if depth == 0:
			my_str_output['str'] += '<ul id=\\"treeresults\\" class=\\"treeview\\">'
		else:
	...
	for c in node.children:
		if c.labelIndex in all_nodes:			
			...
			print_HTML_tree_view(my_str_output, depth+1, c, all_nodes, format_type, id2cat)
...
class MyHandler(BaseHTTPServer.BaseHTTPRequestHandler):	
	...
	def do_GET(self):				
		...
		if self.path.find('?') > -1:						
			...
			#start load example from MySQL
			from svm_checker_helper import query_a_page_from_db			
			bad_request = False						
			start_download = time()
			example =  query_a_page_from_db(uid)
			end_download = time()
			if example == '':
				bad_request = True
			print example	
			if bad_request == True:
				...
				print 'Finishing loading example ' +uid+ ' in ' + ('%.3f' % (end_download - start_download)) + ' seconds'
				...
				var_example = 'example_' + suffix_lst[0]
				var_example_weights = 'example_weights_' + suffix_lst[5]
				var_svm_weights = 'svm_weights_' + suffix_lst[6]
				...
				try:						
					...
					mec[var_example] = example					
					mec.execute(var_probs + ', ' + var_svm_len + ' = webserver_example_based_multi_label_prediction_with_platts_pav_calibration_feature_shrinkage('+var_example+', '+var_predicted_cats+', my_voc_maps, my_idfs, my_models, my_prob_model_choice, my_platts, my_pav_models, predict_param)')											
					#mec.execute(var_probs + ', ' + var_svm_len + ',' + var_example_weights + ',' + var_svm_weights + ' = adv_webserver_example_based_multi_label_prediction_with_platts_pav_calibration_feature_shrinkage('+var_example+', '+var_predicted_cats+', my_voc_maps, my_idfs, my_models, my_prob_model_choice, my_platts, my_pav_models, predict_param)')
					
					#gather probability
					all_probs = mec.gather(var_probs)
					#all_example_weights = mec.gather(var_example_weights)
					#all_svm_weights = mec.gather(var_svm_weights)					
					...
					mec[var_example_weights] = None
					mec[var_svm_weights] = None
				...
				if bad_request == False:						
					...
					if method == 'faf':
						...
						#make tree view
						my_str_output = {'str':''}
						setup_tree_view(my_str_output, root, dict_all_probs, format_type, id2cat, False)
						...
						#make tree view
						my_str_output = {'str':''}
						setup_tree_view(my_str_output, root, dict_all_probs, format_type, id2cat, True)
...
if __name__ == '__main__': 			
	...
	print 'starting webserver at port 8000 ...'	
