		...
		if self.path.find('?') > -1:						
			...
			if bad_request == True:
				...
				try:						
					...
					mec.execute(var_probs + ', ' + var_svm_len + ', v1, v2 = adv_webserver_example_based_multi_label_prediction_with_platts_pav_calibration_feature_shrinkage('+var_example+', '+var_predicted_cats+', my_voc_maps, my_idfs, my_models, my_prob_model_choice, my_platts, my_pav_models, predict_param)')											
					print 'xx'
...
def init_multilabel_models(cats):		
	...
	mec.execute('from svm_checker_helper import webserver_example_based_multi_label_prediction_with_platts_pav_calibration_feature_shrinkage')
