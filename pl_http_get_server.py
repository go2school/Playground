...
class MyHandler(BaseHTTPServer.BaseHTTPRequestHandler):	
	...
	def do_GET(self):
		...
		if self.path.find('?') > -1 and self.path[:6] == '/?faf=':			
			import urllib 
			url = self.path[6:]			
			...
			mec.execute('probs, len_svm_vectors = multi_label_prediction_with_platts_calibration_feature_shrinkage(svm_vector, predicted_cats, my_voc_maps, my_idfs, my_models, my_platts, predict_param)')			
			all_probs = mec.gather('probs')		
			svm_length = mec.gather('len_svm_vectors')			
			print svm_length	
						
			#merge request
			str_probs = ['"' + id2cat[str(prob[0])] + '":' + ('%.6f' % prob[1]) for prob in all_probs]	
			result = '{"id":' + str(0) + ', "probs":{'	+ ','.join(str_probs) + '}}'			
		...
		#output probability larger than threshold
		elif self.path.find('?') > -1 and self.path[:6] == '/?fff=':			
			import urllib 
			url = self.path[6:]			
			url = urllib.unquote_plus(url)
			print url					
		...
		#output tree view directly
		elif self.path.find('?') > -1 and self.path[:6] == '/?fat=':			
			import urllib 
			url = self.path[6:]			
			url = urllib.unquote_plus(url)
			print url					
			...
			result = '{"id":' + str(0) + ', "content": "'  + my_str_output['str'] + '"}'
			print result 
		...
		#output probability larger than threshold
		elif self.path.find('?') > -1 and self.path[:6] == '/?hff=':			
			import urllib 
			url = self.path[6:]			
			url = urllib.unquote_plus(url)
			print url					
		...
		#output tree view directly
		elif self.path.find('?') > -1 and self.path[:6] == '/?hft=':			
			import urllib 
			url = self.path[6:]			
			url = urllib.unquote_plus(url)
			print url					
