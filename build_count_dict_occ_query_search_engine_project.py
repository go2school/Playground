...
#build dictionary		
def extract_term_freq_new(docs, stop_words, cur_dict, cur_raw_dict, voc2stem, stem2voc):
	from nltk.probability import FreqDist
	from nltk.tokenize import regexp_tokenize	
		...
		if len(title)>0:
			...
			for c in cp:
				...
				if arg_term not in cur_dict:
					...
					cur_dict[arg_term] += 1
			#compute raw word freq
			cp = FreqDist(title_removed).items()
			for c in cp:
				arg_term = c[0]
				if arg_term not in cur_raw_dict:
					cur_raw_dict[arg_term] = 1
				else:
					cur_raw_dict[arg_term] += 1
		...
		if len(body)>0:
			...
			for c in cp:
				...
				if arg_term not in cur_dict:
					...
					cur_dict[arg_term] += 1
			#compute raw word freq
			cp = FreqDist(body_removed).items()
			for c in cp:
				arg_term = c[0]
				if arg_term not in cur_raw_dict:
					cur_raw_dict[arg_term] = 1
...
def pl_build_term_freq_list_new(IDs, trunk_size, stop_set, cur_dict, cur_raw_dict, voc2stem, stem2voc):	
	from time import time
	from help_tools import chunks
	...
	for trunk in all_trunks:
		...
		extract_term_freq_new(docs, stop_set, cur_dict, cur_raw_dict, voc2stem, stem2voc)
		print 'finish extract terms from ' + str(start_id) + ' to ' + str(end_id)		
	end = time()
	print 'Finish tasks in ' + str(int(end - start)) + ' seconds'	
...
def write_down_dict(cur_dict, cur_raw_dict, voc2stem, stem2voc, fname_dict, fname_raw_dict, fname_voc_2_stem, fname_stem_2_voc):
	...
	write_dict_with_occ(cur_raw_dict, fname_raw_dict)	
...
if __name__ == '__main__':				
	...
	base_dict_fname = 'query_bing_dictionary0705.txt'
	base_raw_dict_fname = 'query_bing_raw_dictionary0705.txt'
	base_voc2stem_fname = 'query_bing_voc2stem.txt'
	base_stem2voc_fname = 'query_bing_stem2voc.txt'
	base_folder = '/home/mpi/shareddir/query_bing_dictionary2'
	#base_folder = '.'
	...
	mec.execute('from build_count_dict_occ_query_search_engine_project import *')
	...
	mec['base_raw_dict_fname'] = base_raw_dict_fname
	
	mec.scatter('my_id', rc.ids)
	
	#init dictionary
	mec.execute('cur_dict = {}')
	mec.execute('cur_raw_dict = {}')
	...
	for trunk in local_trunks:	
		...
		mec.execute('pl_build_term_freq_list_new(local_trunk, local_trunk_size, stop_words, cur_dict, cur_raw_dict, voc2stem, stem2voc)')
		print 'end extract svm features into dictionary'
		end1 = time()
		print 'Finish trunk ' +str(m_trunk) + '/' + str(n_trunk)+ ' in ' + str(int(end1 - start1)) + ' seconds'
		print '...'
		m_trunk += 1	
	mec.execute('write_down_dict(cur_dict, cur_raw_dict, voc2stem, stem2voc, base_folder + "/" + str(my_id[0]) + "_" + base_dict_fname, base_folder + "/" + str(my_id[0]) + "_" + base_raw_dict_fname, base_folder + "/" + str(my_id[0]) + "_" + base_voc2stem_fname, base_folder + "/" + str(my_id[0]) + "_" + base_stem2voc_fname)')
	end = time()
	print 'Finish tasks in ' + str(int(end - start)) + ' seconds'
